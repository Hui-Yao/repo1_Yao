...
ha6 = dog1.index(123)   #6.index(element)——输出某元素在列表红的索引；如不存在则报错
...
dog1.pop(0)        #8.pop（index）——删除列表中指定索引的元素并换回该项；默认删除最后一个item
print(dog1)        #如果索引超出范围或则列表为空则报错
